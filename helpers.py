...
def plot_image(img, width, height, grayscale=False):
...
# Useful for stratified random sampling
def bin_data(X, y):
    Xb={}
...
# Image generator - create lots of data 
...
# Same as image generator but returns data
# in format for training - flatten images
# and one-hot encode image class
def data_generator(Xb, n):
    n_classes = len(Xb)
    while True:
        X=[]
        y=[]
        for i in range(0,n):
            ...
            n_images = len(Xb[idb])
            idx = np.random.randint(0,n_images)
            img = normalize_grayscale(np.array(transform_image(Xb[idb][idx]), dtype=np.float32).flatten())
            y_hot = np.zeros((n_classes), dtype=np.float32)
            y_hot[idb] = 1.0
            X.append(img)
            y.append(y_hot)
        yield (X, y)
